<div class="lightbox" aria-hidden="true">
    <button class="lightbox-close">✕</button>
    <img class="lightbox-img" src="" alt="">
</div>

<footer>
    <ul class="footer-links">
        <li class="footer-link">
            <a href="https://www.instagram.com/hammarensgraf_"><img src="<?php echo get_template_directory_uri(); ?>/assets/icons/instagram-2-1-logo-svgrepo-com.svg">
            </a>
        </li>
        <li class="footer-link">
            <a href="https://www.linkedin.com/in/alma-hammar-0336011b4/"><img src="<?php echo get_template_directory_uri(); ?>/assets/icons/linkedin-icon-2-logo-svgrepo-com.svg">
            </a>
        </li>
        <li class="footer-link">
            <a href="mailto:almahammar@hotmail.com" class="footer-contact-link">
                <img src="<?php echo get_template_directory_uri(); ?>/assets/icons/email.svg">
            </a>
        </li>
        <li class="footer-link">
            <a href="tel:+46764143363" class="contact-link">
                <img src="<?php echo get_template_directory_uri(); ?>/assets/icons/phone.png">
            </a>
        </li>
    </ul>

    <div class="date">
        <p>&copy; <?php echo date("Y"); ?> Alma Hammar</p>
    </div>
</footer>

<?php wp_footer(); ?>
</body>

</html>